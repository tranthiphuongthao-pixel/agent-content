...
MASTER_SHEET_ID   = os.environ["MASTER_SHEET_ID"]        # set trong .env
CHECKLIST_SHEET   = "Checklist branding"
OUTPUT_SHEET_GID  = int(os.environ["OUTPUT_SHEET_GID"])  # set trong .env
OUTPUT_SHEET_NAME = "Báo cáo Branding"  # fallback nếu gid không tìm được
...
def get_or_create_sheet(sheets_svc, title):
    """
    Trả về (sheetId, sheetTitle) của output sheet.
    Ưu tiên tìm theo OUTPUT_SHEET_GID, fallback sang tên, tạo mới nếu chưa có.
    """
    meta = sheets_svc.spreadsheets().get(spreadsheetId=MASTER_SHEET_ID).execute()

    # Ưu tiên: tìm đúng tab theo gid
    for s in meta["sheets"]:
        if s["properties"]["sheetId"] == OUTPUT_SHEET_GID:
            sid        = s["properties"]["sheetId"]
            real_title = s["properties"]["title"]
            sheets_svc.spreadsheets().values().clear(
                spreadsheetId=MASTER_SHEET_ID,
                range=f"'{real_title}'",
            ).execute()
            print(f"  📌 Ghi kết quả vào tab '{real_title}' (gid={sid})")
            return sid, real_title
    ...
    for s in meta["sheets"]:
        if s["properties"]["title"] == title:
            ...
            print(f"  📌 Ghi kết quả vào tab '{title}' (tìm theo tên)")
            return sid, title
...
def export_dashboard(sheets_svc, project_name, total_articles,
    ...
    sheet_sid, sheet_title = get_or_create_sheet(sheets_svc, OUTPUT_SHEET_NAME)
    ...
    url = (
        ...
        f"/edit#gid={sheet_sid}"
    )
    print(f"  ✅ Dashboard đã xuất → sheet '{sheet_title}' (gid={sheet_sid})")
    return url
